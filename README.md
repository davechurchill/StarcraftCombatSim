...
There is also a hand-authored Visual Studio solution at `vs\StarCraftCombat.sln`, matching the SparCraft `vs` layout. It contains `StarCraftCombatCore` as a static library and `StarCraftCombatVisualizer` as the runnable SFML GUI project.

## Run

```powershell
.\StarCraftCombat\bin\StarCraftCombat.exe
```
...
```text
StarCraftCombat.exe <scenario-file> <starcraft-data-dir> <map-file>
```
